...
public class ProxyAggBatchingExampleWithTransformer {

    private static final Logger LOGGER = LoggerFactory.getLogger(ProxyAggBatchingExampleWithTransformer.class);

    private static final String GROUP_ID = ProxyAggBatchingExampleWithTransformer.class.getSimpleName() + "-consumer";
...
//        Predicate[] branchPredicates = new Predicate[]{batchIsCompletePredicate, acceptAllPredicate};
        ...
        builder.addStateStore(feedToCurrentBatchStoreBuilder);

        filePartInfoStream
//                .filter(filePartInfoPeeker) //peek at the stream and log all msgs
                .transform(() -> new FilePartBatchTransformer(storeName), storeName)
